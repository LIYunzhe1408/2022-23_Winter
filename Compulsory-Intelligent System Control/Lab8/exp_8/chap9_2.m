...
% define sampling time, transfer function 
ts=0.001;
sys=tf(1000,[1,50,2000]);
% convert model from continous to discrete time
dsys=c2d(sys,ts,'z');
% return numerator and denominator coefficient
...
for j=1:1:4
    h(j)=exp(-norm(xi-c(:,j))^2/(2*b(j)*b(j))); % 高斯基函数
end
un(k)=w'*h'; % 网络输出
